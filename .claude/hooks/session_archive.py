#!/usr/bin/env python3
"""세션 보존·색인 — spec 2026-09-13-하네스-design.md §6.1·§6.2. 판단 없음. 실패해도 exit 0(흔적은 scan.log).
사용: session_archive.py <project_dir> <current_session_id> | session_archive.py --unlogged <days>"""
import datetime, fcntl, glob, json, os, re, sys, time
    ...
    for path in paths:
        ...
        try:  # 세션 하나가 전체 스캔을 죽이지 않게 — 대신 흔적을 남긴다
            ...
            if now - os.path.getmtime(path) < ACTIVE_SEC:
                state = "진행중"  # 병행 세션이 아직 작업 중 — 남이 대신 소급하면 기록이 겹친다
            else:
                state = "기재" if is_logged(sid, texts) else "미기재"
            first = (s["prompts"][0] if s["prompts"] else "").replace("\t", " ").replace("\n", " ")[:60]
            rows.append("\t".join([day, sid, first, str(len(s["files"])), str(len(s["commits"])), state]))
        except Exception as ex:
            errors += 1
            _log(now, "error %s %s" % (sid[:8], type(ex).__name__))
    _write(os.path.join(ARCHIVE, "INDEX.tsv"), "시작일\t세션\t첫지시\t편집\t커밋\t기재\n" + "\n".join(rows) + "\n")
