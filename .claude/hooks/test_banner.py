...
eq("C3 30자 자르기", msg3.split("\n")[1], "📅 09-13 기록 · ⏸️ 대기 1 · 🔴 가나다라마바사아자차카타파하가나다라마바사아자차카타파하가나…")
# 최종 리뷰 반영 + V4(다른 저장소 로컬변경은 소음 — 사용자 결정) — (당시) · 줄 중간 🔴 · 달 넘김 날짜 · 🔴 하나(외 없음) · ① 없는 ▶ · upstream 미설정
msg4, _ = run("T\tx\nR\t🔄 sop-project\t[main] upstream 미설정\nR\t🔄 Rpi5\tbehind 1 로컬변경 \nR\t📌 대기 항목 출처\t작업로그 최신 블록 2026-08-31~09-01 — x\n"
              "R\t⏸ ⚠️ gpio — 🔴 급함\t\nR\t⏸ 🔴 인터락\t\nR\t▶ 다음(당시)\t실물 결선 · 촬영\n")
eq("C4 드문 형태", msg4, "🚀 SOP 가디언\n📅 08-31~09-01 기록 · ⏸️ 대기 2 · 🔴 인터락\n▶️ 다음 실물 결선 · 촬영\n⚠️ sop-project [main] upstream 미설정 · Rpi5 behind 1")
