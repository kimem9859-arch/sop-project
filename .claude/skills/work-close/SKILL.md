...
| **C. 실패·중단** | 결과 자리에 **❌ 실패** / **🛑 중단** + 이유(원인 규명 여부) + 현재 상태 + 재개 조건. 재개할 일이면 §12 🔓 미결 표에 행 | spec·plan 머리 상태 줄 `🛑 중단(날짜) — 이유 · 멈춘 태스크` |

요약은 **한 곳에만** 쓴다 — 여러 자리에 쓰면 복제가 되어 낡는다.

### 2.3 결과 사실의 제자리 (통합문서 첫머리 「📐 문서 규칙」 R5·R6)

측정값 → **`docs/성능검증-저널.md`** 의 §12 새 절(**해당 갈래 끝 · 절 안 순서 = §12 머리 「🧱 절 구성 규약」** · 제목 다음 빈 줄 뒤 인용문 마커 `> ✅ **CURRENT-AS-OF <날짜>**` · 제목은 평문(이모지 금지) · 하위 블록은 `#### (N) 이름`) + **통합문서** §12 머리 `[CURRENT]` 표 + **§12 갈래 색인**(둘 다 통합문서) · 부품 → §14.1 · 핀·채널·차단 대상 → §15 「핀·채널 배정」 표 · 설계값 → 해당 절 · 남은 일 → §12 🔓 미결 표. 수치에는 **조건을 함께** 쓴다.
...
- push 는 **4단계에서만** 한다 — 작업 도중 push 는 사용자가 요청할 때만(CLAUDE.md §2).
