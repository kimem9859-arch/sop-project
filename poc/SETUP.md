...
## 옮길 파일 (이것만)
`poc/roi_hover.py`, `poc/score.py`, `poc/hand_landmarker.task`, `poc/requirements.txt`,
`poc_data/`(clips·ground_truth_segments.csv).
※ `.poc_venv`, `.syslibs`, `run.sh` 는 **이 리눅스 전용** — 가져가지 말 것.
...
:: 모델 파일이 없으면 받기 (poc\hand_landmarker.task)
:: https://storage.googleapis.com/mediapipe-models/hand_landmarker/hand_landmarker/float16/1/hand_landmarker.task

:: (1) 첫 프레임 → poc\rois.json 에 버튼 좌표 입력
python poc\roi_hover.py poc_data\clips\정상01.mp4 --dump-frame0 poc\frame0.png
:: (2) 계측
python poc\roi_hover.py poc_data\clips\ --rois poc\rois.json --out poc\out
:: (3) 채점
python poc\score.py --frames poc\out --gt poc_data\ground_truth_segments.csv --sweep
```
...
pip install -r poc/requirements.txt    # mediapipe ARM 설치 실패 시 버전 조정 필요
# GL 라이브러리 누락 시: sudo apt install -y libgles2 libegl1
python3 poc/roi_hover.py ...            # 사용법은 위와 동일
```
