...
1. `selftest/` 4개 실행 — 기준선과 동일하게 **25개 전부 통과**
   (`test_fsm` 14 · `test_recipe` 5 · `test_hoi_sim` 4 · `test_imports` 2)
...
커밋은 세 개로 나뉜다 — `Rpi5`에 자체점검 이동 1개·문서 이동 1개, `sop-project`에
CLAUDE.md 포인터 갱신 1개(repo가 다르므로 합칠 수 없다). 각각 독립적이라
`git revert` 로 부분 롤백도 전체 롤백도 된다. 실패해도 런타임은 영향권 밖이다.
