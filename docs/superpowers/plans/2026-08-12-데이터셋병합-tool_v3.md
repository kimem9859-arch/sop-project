...
**Tech Stack:** Python 3.12 (`~/rfenv`) · pytest 9.1.1 · Pillow(눈확인 오버레이) · ultralytics(학습, Colab) · google-colab-cli 0.6.0
...
- **파이썬**: 테스트·실행 모두 `~/rfenv/bin/python`. 🔴 시스템 `python3`(3.13)에는 pytest 도 ultralytics 도 없다
...
- **입력 데이터셋 (파이 로컬, repo 밖)**: `~/ds_6tool`(접두어 `6tool`) · `~/ds_mech83`(접두어 `mech83`)
- **출력**: `~/ds_tool_v3` (repo 밖 — 데이터셋은 커밋하지 않는다)
...
Run: `cd /home/pi/sop-project/dev/ai_model/tool_v1 && ~/rfenv/bin/python -m pytest test_build_tool_v3_dataset.py -v`
...
**Files:**
- 코드 변경 없음 (실행·검증 작업)
- 입력: `~/ds_6tool` · `~/ds_mech83` → 출력: `~/ds_tool_v3`

**Interfaces:**
- Consumes: Task 5 의 CLI
- Produces: `~/ds_tool_v3` (repo 밖) + 리포트 수치. 이 수치는 **통합문서 §10.40 에 기록될 것**이나 문서 작업은 세션 마무리에서 한다.
...
~/rfenv/bin/python build_tool_v3_dataset.py ~/ds_tool_v3 ~/ds_6tool:6tool ~/ds_mech83:mech83
```
...
```bash
cd /home/pi/sop-project/dev/ai_model/tool_v1
~/rfenv/bin/python dataset_diversity.py ~/ds_tool_v3 driver wrench pliers
```
...
**Interfaces:**
- Consumes: Task 6 이 만든 `~/ds_tool_v3`
...
```bash
cd /home/pi/sop-project/dev/ai_model/tool_v1
~/rfenv/bin/python preview_labels.py ~/ds_tool_v3 ~/tool_v3_preview 20 0
```
...
Run: `cd /home/pi/sop-project/dev/ai_model/tool_v1 && ~/rfenv/bin/python -c "import ast; ast.parse(open('train_tool_v3.py').read()); print('OK')"`
Expected: `OK`
...
2. **🗂 파이 로컬 자산** 표에 `~/ds_tool_v3`(병합본)·`~/tool_v3_preview`(눈확인) 행 추가
3. 아래 절을 새로 추가:
...
- [ ] `~/ds_tool_v3` 생성 — 약 14,300장 / 출처 약 9,788
