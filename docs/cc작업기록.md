# CC 작업 기록 (Claude Code 활용·인프라)

> 이 프로젝트에서 **Claude Code를 어떻게 쓰고 있으며, 어떤 CC 기능·도구를 추가·개선·유지보수했는지** 모으는 문서.
> 목적은 프로젝트의 실험·연구가 아니라 **Claude Code를 생산적·유용하게 쓰는 법**(전역 CC 인프라 + 이 프로젝트 작업 효율).
>
> **역할 경계 (drift 방지)** — 서로 다른 질문에 답하므로 공존:
> - `cc작업기록.md`(이 파일) = **CC를 어떻게 활용/개선하나** (주제별 큐레이션)
> - `docs/작업로그.md` = **무엇을 언제 어느 세션/커밋으로** 했나 (시간순 raw 저널, CC·프로젝트 섞임 OK)
> - `CLAUDE.md`·통합문서 = 프로젝트 사양·진척
>
> 시간순 이력은 작업로그를 보고, 여기엔 **"어떤 CC 인프라가 있고 어떻게 일하는가 + 재사용 지식"** 만 정리한다.
...
- **세션 시작**: 브리핑 훅이 뜨는 "이어갈 작업 + session_id"와 원격 동기화 상태를 먼저 확인. behind면 pull 먼저.
- **세션 마무리**: **"오늘 마무리"/"세션 정리"** 라고 하면 이번 세션 작업을 4분류(✅완료/⏸중단/▶다음/🔗커밋)로 `docs/작업로그.md`에 **전체 UUID 태그**로 기록.
- **문서 커밋**: pre-commit 정합성 훅이 자동 대조(경고만). 무시하려면 `SKIP_DOCSYNC=1`.
- **큰/되돌리기 어려운 변경**: 계획(plan mode) 또는 A/B 선택지로 먼저 확인.
- **세션 이어가기**: 작업로그의 `session <UUID>`를 `claude --resume`에 사용.
...
- **2026-07-01~02**: 세션 작업로그 시스템 구축(brief/commit 훅 + "오늘 마무리" 의식) · 문서 정합성 pre-commit 훅(LLM diff앵커)·프롬프트 튜닝 · 구 doc-consistency 훅 제거 · 세션ID 전체 UUID 통일 · 브리핑 마커 오탐 수정.
- (세부 커밋·시간순은 `docs/작업로그.md` 참조)
