...
- 이런 장비에는 인터락으로 보호되는 위험 구역이 있다 [B].
- 산업 설비의 트랩키 인터락은 열쇠를 정해진 순서로만 옮길 수 있게 해, 장비를 **미리 정해 둔 안전한 조작 순서대로만** 움직이게 한다 [C].
- → 인터락이 강제하는 것은 **설계 단계에서 정해 둔 조건과 순서**다.

#### ③ 정비 때는 그 인터락을 해제할 수 있다
- 반도체 장비 안전 가이드라인 SEMI S2는 인터락으로 보호되는 구역에 정비 접근이 필요할 때 **의도적 조작을 거쳐 인터락을 해제하는 것을 허용**한다 [B].
...
반영 전 확인: [C] 트랩키 출처(Kirk Key 백서 유지 / ISO 14119:2024 — 사용자 판단) · [G] NIOSH 페이지는 루프 재대조 때 접근 차단(기존 통합문서 원문 대조 기록에 기댐)
