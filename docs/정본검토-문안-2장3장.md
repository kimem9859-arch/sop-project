...
- → 1인칭 비전으로 절차 오류를 판정하는 **문제 설정 자체는 이미 연구돼 있다.** 다만 이 연구들이 다루는 것은 **판정·검출 방법까지**이며, 작업에 개입하는 단계는 다루지 않는다.
...
- → 비전 판정을 차단에 연결하는 것 자체는 ②의 생산 정지 기능과 함께 새롭지 않다. 다만 여기서 판정 대상은 **부품·제품의 상태**이지 작업자의 행동 순서가 아니다.

#### ④ 웨어러블 — 스마트글라스 작업 지원
- 스마트글라스에 정비 관리 시스템을 연동해 **작업 허가·격리 확인을 화면에 띄우고, 음성 명령으로 작업 지시를 진행하는** 제품이 있다 [S].
- → 1인칭 웨어러블로 절차를 안내하는 조합은 제품으로 나와 있다. 다만 작업 진행은 **작업자의 음성·시선 입력**으로 이루어지며, 비전이 작업자의 행동을 보고 판정하지 않는다.

#### ⑤ 종합 — 각 사례가 다루는 범위

| 사례 | 다루는 것 | 다루지 않는 것 |
| --- | --- | --- |
| ① 학계 1인칭 오류 검출 | 1인칭 판정 · (일부) 작업 도중 판정 | 작업 개입 |
| ② 상용 작업 단계 감시 | 단계 판정 · 즉시 경고 · (일부) 생산 정지 | 1인칭 · 정비 작업 |
| ③ 비전 검사 + 차단 | 물리적 차단 | 사람의 행동 순서 |
| ④ 스마트글라스 작업 지원 | 1인칭 · 웨어러블 | 비전 판정 |

- → 조사 범위 안에서 **1인칭 판정·순서 경고·조작 차단·웨어러블 네 요소를 정비 작업에 함께 적용한 사례는 찾지 못했다.** 찾지 못한 것이 없다는 증명은 아니다. 본 프로젝트가 이 사례들과 어떻게 다른지는 **§3.3** 에서 정리한다.
