...
| 개발환경(버전·스택) | 통합문서 **§14.2** ★ 정본 / 개발보고서용 시점 사본 = `~/hanium-문서/hanium-장려상/개발보고서/개발보고서-개발환경.md` |
| 개발보고서·제작설계서 작업 자료 (본문 변환본·소재·그림·생성도구) | 형제 저장소 `~/hanium-문서/hanium-장려상/` (아래 「형제 저장소」) — 수치 정본은 통합문서 §12 `[CURRENT]` |
...
### 형제 저장소 — `hanium-문서` (한이음·학교 프로젝트 문서 작업)

옛 `hanium-docs` 의 역할을 이어받는 **로컬 git 저장소**다(옛 파일은 가져오지 않았다). 안에 작업공간이 둘이다.

```
/home/pi/sop-project/                ← 여기 (통합문서·코드 = 사실의 정본)
/home/pi/hanium-문서/                ← 형제 저장소
...
- 의존은 **한 방향뿐** — 이 저장소의 코드·스크립트는 그쪽을 참조하지 않는다. ⚠️ 예외 하나 = 세션 시작 배너가 `../hanium-문서/hanium-장려상/계획.md` 의 `### 일정` 절을 읽어 **마감 D-day** 를 띄운다. 🛑 **지금은 그 파일이 없어 D-day 가 안 뜬다**(없으면 생략하는 설계라 오류는 아니다).
- 사실·수치가 바뀌면 **이 저장소에서 먼저** 고친다.
- 📦 옛 작업공간 `hanium-docs` 는 1차 접수(9/8)용이었고 역할이 끝나 **로컬에서 지웠다**. 이력은 원격 private 저장소에 35커밋으로 남아 있고, 로컬 사본은 `~/lab/hanium-docs-백업-20260922` 다. 9/22 에 만들었다 지운 장려상 작업공간의 백업은 `~/lab/hanium-장려상-백업-20260922` 다(둘 다 `hanium-문서` 로 가져오지 않았다).
