...
| 측정 **절 본문** — 언제 어떤 조건에서 나왔나 | `docs/성능검증-저널.md` 의 해당 갈래 끝에 새 절(R6) |
...
**규칙 문서는 「어디에 두나」가 「무엇을 쓰나」만큼 중요하다** — 자리가 곧 **언제·어느 머신에서 읽히는가**를 정한다. ⏸ **규칙 문서를 관리하는 규칙은 지금 없다** — 경위·길이·중복·폐기 이관이 공백이다.

| 규칙 공간 | 언제 읽히나 | git 공유 | 쓰는 것 |
| --- | --- | --- | --- |
| `CLAUDE.md` | **매 세션** | ✅ | 항상 지켜야 하는 규칙·금지·함정 |
| `Rpi5/CLAUDE.md` | **`Rpi5/` 파일을 읽을 때만** | ✅ | 런타임 코드 맥락·그 코드의 함정 |
| `.claude/skills/` | **이름·설명은 매 세션**, 본문은 호출할 때 | ✅ | 작업별 절차(이어하기·마무리 등) |
| `.claude/rules/` (`paths:` 없음) | **매 세션** — `CLAUDE.md` 와 같은 우선순위 *(현재 미사용)* | ✅ | 항상 걸리는 주제별 규칙 |
| `.claude/rules/` (`paths:` 있음) | 그 글롭에 맞는 파일을 읽을 때 *(현재 미사용)* | ✅ | 특정 문서·폴더에만 걸리는 규칙 |
| `.claude/settings.json` | **항상 — 사람이 아니라 기계가 강제** | ✅ | 권한 deny · 훅 |
| **auto memory** | 색인(`MEMORY.md`)만 매 세션, 본문은 필요할 때 | 🔴 **안 됨 — 머신 로컬** | 선호·경위. **다른 머신은 못 본다** |
| `CLAUDE.local.md` | 매 세션 | 🔴 안 됨 (gitignore) | 그 머신에만 해당하는 개인 설정 |
| `../hanium-docs/CLAUDE.md` | 🔴 **읽히지 않는다**(형제 폴더) | ✅ | **규칙을 두지 않는다** |

🔴 **auto memory 는 머신 로컬이다** — 파이1에 저장한 것을 데스크톱은 보지 못한다. **양 머신이 지켜야 할 규칙은 memory 가 아니라 git 에 올라가는 곳**에 둔다.
...
docx 바이너리를 이 저장소에서 떼어내려고 갈랐다.

```
/home/pi/sop-project/     ← 여기 (통합문서·코드 = 사실의 정본)
/home/pi/hanium-docs/     ← 제출문서 (Word·PPT 도구·다이어그램·서식)
```

- 🔴 **`hanium-docs` 는 정리 전까지 규칙·내용 모두 참고하지 않는다**(CLAUDE.md §5). 사실은 **통합문서에서** 가져온다.
- 🔴 **그 저장소에 규칙을 두지 않는다** — 형제 폴더라 cwd 도 상위도 아니어서 **어느 머신에서도 자동 로드되지 않는다.** 거기 쓴 규칙은 아무도 읽지 않는다.
- 두 폴더는 **양 머신 모두** 형제로 둔다. 상대경로로 정본을 읽어야 하면 `../sop-project/docs/통합문서.md` 다.
...
| --- | --- | --- | --- |
| 역할 | 설계·통합문서 편집 · PoC(x86) · 발표 제작 | Rpi5 코드 실행·실HW 테스트·Hailo · 통합문서 편집 | 음성비서 「말하는 뇌」(LLM·STT·TTS) 예정 |
| 클론 | `sop-project` | `sop-project` + 그 안 `Rpi5/`(엄브렐러) | **없음** (2026-08-15 새 카드로 재구성 — 복제본 아님) |
| 접속 | — | — | `ssh pi2` (Tailscale `100.67.150.74` — 공유기 IP 는 DHCP 로 바뀐다, 2026-09-14 확인 `192.168.1.12`) |

통합문서는 양쪽에서 편집·사용한다(파이 = 개발·테스트 중 수정 / 데스크톱 = 발표 제작).

> ⚠️ **sop-pi-2는 2026-08-15부터 Pi#1 복제본이 아니다.** 새 SD카드에 OS·한국어 환경·기본 개발도구만 세웠고 **`sop-project`·`Rpi5`·Hailo·venv가 없다.** 경위·카드 사양 = `docs/작업로그.md` 2026-08-15 블록.
