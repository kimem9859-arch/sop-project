...
| 개발환경(버전·스택) | 통합문서 **§14.2** ★ 정본 / 개발보고서용 시점 사본 = `~/project-docs/hanium-장려상/개발보고서/개발보고서-개발환경.md` |
| 개발보고서·제작설계서 작업 자료 (본문 변환본·소재·그림·생성도구) | 형제 저장소 `~/project-docs/hanium-장려상/` (아래 「형제 저장소」) — 수치 정본은 통합문서 §12 `[CURRENT]` |
...
| 파이 도구·함수 목록 · 조사 근거 | `Rpi5/도구목록.md` · `Rpi5/함수목록.md` · `Rpi5/조사/` |
...
/home/pi/project-docs/                ← 형제 저장소
├─ hanium-장려상/                     한이음 공모전 — 개발보고서·제작설계서 작업 자료 · 장려상 평가 준비
├─ 융합프로젝트/                      학교 융합프로젝트 과정 문서
└─ 백업/hanium-docs/                  옛 작업공간 — 참고용만 (규칙 낡음)
...
- 🔴 **그 저장소는 sop-project 세션에서 로드되지 않는다**(형제 폴더). 문서 작업은 **그 작업공간 폴더에서 세션을 연다.**
- 의존은 **한 방향뿐** — 이 저장소의 코드·스크립트는 그쪽을 참조하지 않는다. ⚠️ 예외 하나 = 세션 시작 배너가 `../project-docs/hanium-장려상/계획.md` 의 `### 일정` 절을 읽어 **마감 D-day** 를 띄운다. 🛑 **지금은 그 파일이 없어 D-day 가 안 뜬다**(없으면 생략하는 설계라 오류는 아니다).
- 사실·수치가 바뀌면 **이 저장소에서 먼저** 고친다.
- 📦 옛 작업공간 `hanium-docs` 는 1차 접수(9/8)용이었고 역할이 끝나 **로컬에서 지웠다**. 이력은 원격 private 저장소에 35커밋으로 남아 있고, 로컬 사본은 `~/project-docs/백업/hanium-docs/` 다(`CLAUDE.md`·`.claude/` 는 하위 폴더 자동 로드를 막으려 이름만 바꿨다 · 같은 사본이 `~/lab/hanium-docs-백업-20260922` 에도 있다). 9/22 에 만들었다 지운 장려상 작업공간의 백업은 `~/lab/hanium-장려상-백업-20260922` 다(가져오지 않았다).
- 🔑 **규칙을 새로 쓸 때의 기준** — `hanium-docs` 의 규칙·판정표는 **8월 말에 멈춰 낡았다**(판정표 531줄 중 21곳이 옛 절 번호를 가리킨다). 기준은 저장소가 아니라 **이미 제출한 최종본 PDF** 이고, 집필 규칙은 **개발보고서에만** 적용한다(제작설계서는 ppt 라 제외).
