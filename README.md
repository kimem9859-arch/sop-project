...
**규칙 문서는 「어디에 두나」가 「무엇을 쓰나」만큼 중요하다** — 자리가 곧 **언제·어느 머신에서 읽히는가**를 정한다. 편집 규율은 통합문서 「📕 규칙 문서 지침」 **G1~G4**.

| 규칙 공간 | 언제 읽히나 | git 공유 | 쓰는 것 |
| --- | --- | --- | --- |
| `CLAUDE.md` | **매 세션** | ✅ | 항상 지켜야 하는 규칙·금지·함정 |
| `Rpi5/CLAUDE.md` | **`Rpi5/` 파일을 읽을 때만** | ✅ | 런타임 코드 맥락·그 코드의 함정 |
| `.claude/skills/` | **호출할 때만** | ✅ | 작업별 절차(이어하기·마무리 등) |
| `.claude/rules/` | `paths:` 가 맞는 파일을 읽을 때만 *(현재 미사용)* | ✅ | 특정 폴더에만 해당하는 규칙 |
