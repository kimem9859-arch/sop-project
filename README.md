...
- 📦 옛 작업공간 `hanium-docs` 는 1차 접수(9/8)용이었고 역할이 끝나 **로컬에서 지웠다**. 이력은 원격 저장소(private · 보관 = 읽기 전용)에 35커밋으로 남아 있고, 로컬 사본은 `~/project-docs/백업/hanium-docs/` 다(`CLAUDE.md`·`.claude/` 는 하위 폴더 자동 로드를 막으려 이름만 바꿨다). 9/22 에 만들었다 지운 장려상 작업공간의 백업은 `~/lab/hanium-장려상-백업-20260922` 다(가져오지 않았다).
