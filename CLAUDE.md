...
- **자동 점검 인프라(훅)**: SessionStart ① `session-worklog-brief.sh`(**세션 시작 표 배너** = 원격 동기화 + ⏸중단·▶다음 이어하기 + 현재 session_id 주입을 한 표로 표시; 내부에서 `session-sync-check.sh`를 **순차 호출**[SessionStart 병렬실행 경쟁 회피]하고 `_banner.py`로 CJK정렬 표+systemMessage 렌더) ② `session-prune-stubs.sh`(resume 목록의 tiny/원격제어 스텁 세션을 `.trash`로 정리, systemMessage 배너). SessionEnd ③ `session-worklog-commit.sh`(그 세션이 만든 커밋을 `docs/작업로그.md`에 자동 매핑). **pre-commit ④ `.githooks/pre-commit`**(문서 정합성 — `CLAUDE.md`·`README.md`·`docs/*.md` 커밋 시 **diff(-U20 컨텍스트)만** `claude -p`(Haiku)에 보내 diff 내 모순을 경고. **warn-only·비차단**, `SKIP_DOCSYNC=1`로 스킵. ※2026-07-14 Fable 점검: 문서 전문+memory 전송을 diff-only로 축소[비용·인젝션 면적] — 교차 문서 대조는 포기, 얕은 안전망). ①②③은 `.claude/settings.json` 등록, ④는 **클론 후 1회 `git config core.hooksPath .githooks` 필요**(`.githooks/README.md`). ④는 구 `doc-consistency-check.sh`(SessionStart grep·2026-07-01 제거)를 **다른 설계로 대체** — 타이밍(세션시작→커밋시점)·LLM 의미대조·diff앵커로 교정. 문서 drift의 1차 방어는 여전히 **아래 단일정본 규칙 + 편집 규율**, ④는 그 안전망. 권한 허용목록은 `.claude/settings.json`. **전부 repo 공유**(양 머신 동일).
...
- ✅ **console_v2 학습(§10.14)·`.hef` 변환(§10.15) 완료(2026-07-16)** — 데스크톱 RTX 5060+WSL2. 파랑 B4 데이터셋 652장 재학습(153ep, 9.6분) → DFC **level 1**·캘리브 **652**(v1은 level 0·64) → `console_v2.hef` 4.4MB **Rpi5 `Demo/models/` 배포 완료**. HAR 검증(uint8·NMS·5클래스) 통과. 절차 정본 = `dev/ai_model/console_v2_학습가이드.md`.
...
- ✅ **⑤ replay 평가 완료(2026-07-16, 파이 — 수치 §10.17)** — 5세션 전량. **판정 기준 = §10.16**(“v1 대비 상승”은 쓰지 않음). ①**하한선 통과**(B4 83.8~92.5%, 롤백 사유 없음) ②⭐**저조도 붕괴 소멸** — v1에서 B3 0%·EMO 반감이던 저조도(`180016`)가 v2에선 B3 92.5%·B4 92.5%로 **기준선보다 오히려 높음** → §10.13 "저조도 파랑 B4 생존 미측정" 플래그 **해소** ③누출-프리 세션(`173728`)도 B4 94.3%. ⚠️ 남는 신호: **B4 confidence 마진이 저조도에서 최저**(median 0.802). ③`.hef` vs `.pt` 대조는 파이 불가 → 미실시.
- 🔴 **그래도 "B4 해결" 미선언** — ⑤ 통과는 **약한 증거**(학습에 쓴 장면·`replay_raw`엔 정답 없음). v1도 mAP·HAR은 통과했으나 실추론 0회였다(§10.5~§10.7).
- 🔴 **▶ 최종 판정의 유일한 근거 = test 세션 촬영(미실시)** — 다른 날·조명·실콘솔, **파랑 스티커 동일 사양 필수**. 절반은 train 보충용 / 절반은 손대지 않는 test용으로 분리.
