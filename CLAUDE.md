...
- 🔴 **▶ 최우선 = ⑤ replay 평가(파이) — 단 이것도 최종 판정은 못 한다** — **판정 기준 정본 = §10.16**. `test/raw/<세션>`이 곧 학습 데이터의 출처라 **학습에 쓴 이미지로 시험 보는 구조**이고, **"v1 대비 검출률 상승"은 무의미**(v1은 파랑 미학습이라 0%가 예정, v2는 그 프레임으로 학습). 유효한 것 = ①하한선 ②**세션 간 비교**(저조도만 무너지면 조명 탓) ③`.hef` vs `.pt` 대조.
