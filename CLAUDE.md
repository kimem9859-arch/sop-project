...
- **🔴 정반사 → B1·B3가 B2로 오분류(§10.8, 2026-07-13 재검증·정정)**: B1·B2·B3는 형태 동일·**색으로만 구분**되는데 정반사로 색이 날아가면 전부 B2가 됨. **⚠️ USB 고유 문제가 아니다 — 양쪽 카메라 모두 발생**(B2 중복 프레임율: USB 1차 99.1% / **ESP32 1차 37.5%** / 정지장면 2차는 양쪽 0%). **결정 요인은 카메라가 아니라 촬영 각도·조명**이며 USB의 AE는 악화 요인일 뿐. **ESP32의 버튼 채도가 오히려 20~25% 낮아**(B1 118.9 vs 153.4 등) 조건 악화 시 먼저 무너질 수 있음. ※ **`confusion_log`로는 안 잡힘**(처음부터 B2로 인식 → 트랙 전환 없음) = 계측 사각지대, **B2 중복 검출 지표**로 봐야 함. 대응 3축: ①조명 측면·확산(근본) ②console_v2에 정반사 조건 학습 ③중복검출 지표 계측 추가. **🔴 실콘솔(포맥스) 이관 시 재질 변경 → 반사특성 재검증 필수**.
