...
- **▶ 최우선(지금) = 실테스트(실시간 영상)** — **test 이미지와 무게중심이 반대**다: 실테스트가 답하는 **FPS·트래킹 연속성·파이프라인 안정성은 콘솔 종류와 무관**(병목은 ESP32 TCP 경로 §10.11, 콘솔은 피사체일 뿐)이라 **모조로 지금 한 번이 본편**이고, 실콘솔에선 **시연 리허설 + AE×포맥스 재질 반응**만 가볍게 보면 된다. 반대로 test 이미지는 **실콘솔이 본편**. 같이 볼 것: 🔴**FPS 재측정**(현재 12.5<15 NFR-1 미달이고 그마저 **MediaPipe 미포함** 값 — §10.6이 console_v2·MediaPipe 포함 재측정 요구) · **A7**(USB 웹캠 상시 polling 개선, 보류 건) · 저조도 B4 마진(§10.17).
- 🔴 **HOI(손 검출) 여전히 보류** — `hailo_platform`은 Python 3.13 전용인데 **mediapipe가 aarch64/3.13 미지원**. 실테스트를 돌려도 **손-버튼 접촉 판정은 못 돈다**(버튼 검출 + GPIO 입력 FSM만 동작). 비전 기반 순서위반 감지의 한 축이라 **별도 해결 과제**.
