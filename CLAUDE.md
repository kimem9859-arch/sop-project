...
- **▶ 최우선 다음 = 라벨링 → console_v2 재학습** — ①**라벨링(사람, Roboflow 웹)**: B4 전량 추가 + 배경오탐 삭제 + 정반사 오분류 수정(**정답=진짜 버튼 정체**, 노란버튼이 하얗게 보여도 B1) + 저조도 B3 보충 ②**증강 학습**(블러 σ0~2.0·JPEG q30~100·저해상도·정반사 합성 — 고주파 의존 제거, §10.9). ※ **증강은 Roboflow가 아니라 학습 코드에서**(Albumentations) — Roboflow엔 JPEG압축·저해상도·정반사 증강이 **없고**, 오프라인이라 매 epoch 같은 증강본을 봄. **증강의 목적은 양 보완이 아니라 강건성 학습**. ③GPU 재학습·DFC 변환(캘리브 1024+장). 평가 = `replay_raw.py --hef console_v2.hef`.
- **🔴 test 세션 미촬영** — 반드시 **다른 날·다른 조명**으로 별도 촬영, **학습에 단 한 장도 넣지 말 것**(v1의 "같은 영상 랜덤분할 → mAP 0.993인데 실전 0%" 함정 반복 금지, §10.13).
