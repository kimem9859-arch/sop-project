...
- **▶ 최우선 다음 = 라벨링 → console_v2 재학습** — ①**라벨링**(Roboflow, B4 전량 수동 + 정반사 오분류 수정 + 저조도 B3 보충. 정답=진짜 버튼 정체) ②**증강 학습**(블러 σ0~2.0·JPEG q30~100·저해상도·정반사 합성 — 고주파 의존 제거, §10.9) ③GPU 재학습·DFC 변환(캘리브 1024+장). 평가 = `replay_raw.py --hef console_v2.hef`.
